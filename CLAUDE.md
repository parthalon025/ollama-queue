...
ollama_queue/
  ...
  api/                # FastAPI REST API (90+ endpoints, APIRouter per domain)
    ...
    health.py         # /api/health (full telemetry log) + /api/health/status (lightweight probe)
...
- **`DLQManager.handle_failure` holds `db._lock` for the entire body** — prevents the double-retry TOCTOU race. Any code path that calls `handle_failure` must not already hold `db._lock`; since `db._lock` is an RLock the call will not deadlock, but the single-caller atomicity guarantee breaks if the lock is already held at entry.
...
- **`BackendsPanel` inference_mode toggle** — each backend row shows a `GPU` / `GPU+CPU` button. Clicking calls `PUT /api/backends/{url}/inference-mode?mode=X` then re-fetches backends. Env-var backends not yet in the DB are auto-registered on first toggle. The `inference_mode` field is included in `GET /api/backends` response.
- **`inference_mode: 'gpu_only'` routing enforcement** — `_apply_gpu_only_filter()` in `backend_router.py` runs before tier 2 (model availability). For each `gpu_only` backend: fetches `vram_total_gb` from the remote `/api/health` (600s TTL), estimates model VRAM via `OllamaModels().estimate_vram_mb()`, and excludes the backend when `needed_mb > total_gb * 1024 * (1 - vram_pct/100)`. Always fails open (returns original list) if no VRAM data available so requests are never silently dropped.
- **`GET /api/health` returns `vram_total_gb`** — read from `HealthMonitor.get_vram_total_gb()` (nvidia-smi) at startup and cached as `_VRAM_TOTAL_GB`. Required by the remote backend router to compute available VRAM for `gpu_only` filtering. Returns `0.0` on non-GPU machines.
- **`GET /api/health/status`** — lightweight liveness probe: returns `{"ok": true, "daemon_state": "...", "uptime_s": N}` with no telemetry log. Use for uptime checkers and container health checks; response is always under 100 bytes.

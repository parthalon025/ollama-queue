...
- **`useShatter` hook integrates with atmosphere effect budget** — `isOverBudget()` (exported from `stores/atmosphere.js`) is checked at the top of `fire()` — all tiers suppressed when budget exceeded. `trackEffect('shatter')` is called before firing; the `onComplete` callback passed to `shatterElement()` calls the cleanup function from `trackEffect()`. Three tiers: `earned` (7 fragments), `complete` (6), `routine` (3). The hook returns `[ref, fire]` — attach `ref` to the button element, call `fire()` in `onClick`.

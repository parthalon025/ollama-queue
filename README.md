...
| View | Description |
|---|---|
| **Now** | Host-first command center: HostCard list (one per GPU backend — current job/eval/model, VRAM/CPU gauges with gradient color ramp, data-mood cascading, ShGlitch on health transitions, ShThreatPulse on offline backends, ShFrozen on stall elapsed; expand each card for VRAM % 24h trend + system health gauges full-width). ShStatsGrid KPI section with ShFrozen staleness (30s/2m/5m thresholds). DLQ dismiss wired to useShatter('earned') with 7-fragment shatter. Terminal voice ALL CAPS throughout. Alert strip with auto-disabled recurring jobs count. GPU activity heatmap (24h×7d) at bottom of page. |
...
| **Perf** | Per-model performance table, cross-model performance curve chart (SVG scatter, log-scale, log-linear regression line + 90% CI band), ShTimeChart showing RAM % trend (last 24h), per-backend throughput table showing tok/min per GPU |

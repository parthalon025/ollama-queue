...
| REST API | 60+ endpoints covering queue management, scheduling, health, settings, eval pipeline |
| Web dashboard | 6-view Preact SPA: Now (burst regime badge), Plan (24h Gantt), History (preemption indicators), Models, Settings (12+ tunable parameters), Eval (runs, variants, trends, settings) |
...
tests/                         # 590 tests (pytest-xdist parallel)
```
...
```bash
source .venv/bin/activate
pytest  # 590 tests, parallel by default
```
